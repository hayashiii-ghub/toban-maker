...
4. **テンプレート詳細ページに「関連テンプレート」モジュールが無い** — 実ユーザーの回遊と bot HTML の内部リンク密度が薄い
...
### Section 3: 関連テンプレートモジュール

**3-1. 実ユーザー向け（React コンポーネント）**

**変更対象**: `client/src/pages/TemplateDetailPage.tsx`

ページ末尾に「同カテゴリの他のテンプレート」セクションを追加:

```tsx
<section aria-labelledby="related-templates">
  <h2 id="related-templates">同カテゴリの他のテンプレート</h2>
  <ul>
...
データソース:
- 現在表示中のテンプレートの `categoryId` を取得
- `TEMPLATE_SEO_DATA` から同 `categoryId` の他テンプレートを抽出（自身を除く）
- 上位 3〜4 件を表示

スタイル: 既存テンプレート一覧（`TemplatesPage.tsx`）のカードスタイルを再利用。
...
| `client/public/og-image.png` | 新規ファイル（1200×630 PNG） |
| `client/src/pages/TemplateDetailPage.tsx` | 関連テンプレートセクション追加 |
